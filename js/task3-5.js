...
function getAllPropValues (array, prop) {
    ...
    for (const product of array) {
        if (product[prop] !== undefined){ //перебрать массив, и push только если есть заданый ключ, если такого ключа(поля) нет, чтобы не пушило undefined - задаем условие !== undefined;
             valueOfproperty.push(product[prop])
        }
    }
